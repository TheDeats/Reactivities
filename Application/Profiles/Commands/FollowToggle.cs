...
public class FollowToggle
{
    ...
    public class Handler(AppDbContext context, IUserAccessor userAccessor) : IRequestHandler<Command, Result<Unit>>
    {
        ...
        {
            User? observer = await userAccessor.GetUserAsync();
            User? target = await context.Users.FindAsync([request.TargetUserId], cancellationToken);
        if (target == null)
            {
                return Result<Unit>.Failure("Target user not found", 400);
            }

            UserFollowing? following = await context.UserFollowings.FindAsync([observer.Id, target.Id], cancellationToken);
            if (following == null)
            {
